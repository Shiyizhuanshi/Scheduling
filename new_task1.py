...
def choose_job_to_schedule(jobs_to_choose, cur_time):
    min_cost = 999999
    job_to_schedule = -1
...
def schedule_last_jobs(edges, scheduled_jobs, cur_time):
    last_jobs = []
    for edge in edges:
    ...
    if len(last_jobs) == 0:
        ...
        while (len(last_jobs) > 0):
            ...
            job = choose_job_to_schedule(jobs_to_choose, cur_time)
...
def calculate_max_cost(schedule_jobs):
    print("calculate_max_cost for schedule_jobs:" + str(schedule_jobs))
    max_cost = -999999
    cur_time = 0
    for job in schedule_jobs:
        cur_time += p[job]
...
def calculate_best_schedule(egdes_of_jobs):
    ...
    while(len(scheduled_jobs) != 31):
        ...
        if (len(jobs) == 0):
            schedule_last_jobs(egdes_of_jobs, scheduled_jobs, total_completion_time)
        else:
            job = choose_job_to_schedule(jobs, total_completion_time)
            schedule_job(job, egdes_of_jobs)
            total_completion_time -= p[job]
            scheduled_jobs.insert(0, job)
